...
#include"vsh.h"
#define MAX 128
char *intcmds[]={"ls","hist","curpid","ppid","cd","quit","cat","calc","cal","ntow","style","wc","help"};
...
if(state==1)
{
 ...
 for(i=0; i<j; i++)
 {
  ...
  if(strcmp(cm[i],">")==0)
  {
   if((fd2=open(cm[i+1],O_WRONLY | O_TRUNC | O_CREAT , 0664 ))==-1)
    {
     perror("open");
     dup2(stdi,0);
     dup2(stdo,1);
...
/*************** Executing  External  Commands ******************/
 ...
 { 
  ...
  switch(j)
  {
  case 1:execlp(cm[0],cm[0],0);break;
  case 2:execlp(cm[0],cm[0],cm[1],0);break;
  case 3:execlp(cm[0],cm[0],cm[1],cm[2],0);break;
  case 4:execlp(cm[0],cm[0],cm[1],cm[2],cm[3],0);break;
  case 5:execlp(cm[0],cm[0],cm[1],cm[2],cm[3],cm[4],0);break;
  case 6:execlp(cm[0],cm[0],cm[1],cm[2],cm[3],cm[4],cm[5],0);break;
  case 7:execlp(cm[0],cm[0],cm[1],cm[2],cm[3],cm[4],cm[5],cm[6],0);break;
  case 8:execlp(cm[0],cm[0],cm[1],cm[2],cm[3],cm[4],cm[5],cm[6],cm[7],0);break;
  case 9:execlp(cm[0],cm[0],cm[1],cm[2],cm[3],cm[4],cm[5],cm[6],cm[7],cm[8],0);break;  
  case 10:execlp(cm[0],cm[0],cm[1],cm[2],cm[3],cm[4],cm[5],cm[6],cm[7],cm[8],cm[9],0);break;
  }
   printf("%s: No Such Command...\n",cm[0]);
   printf("Try \'help\' for Internal Commands\nTry \'info\' for External Commands\n");
   _exit(0);
  }
 }
}
  ...
  while((dnt=readdir(dp))!=0)
  {
   ...
   else
   {
     if(stat(dnt->d_name,&stbuf)==-1)
       perror("stat");
     printf("%9d %8d %4d   %.15s\n",dnt->d_ino,stbuf.st_size,stbuf.st_nlink,dnt->d_name);
   } 
  }
  printf("\n");
  closedir(dp); 
...
void help()
{
  printf("      $ vsh                         - For Starting the vsh Shell\n");
  printf("      $ vsh [-c] [cmdname][args]    - For Executing vsh Shell's Commands\n\n");
  printf("******* Commands of vsh Shell *******\n\n");
  printf("ls     - List the Directory & Files\nhist   - Print History of Commands\n");
  printf("curpid - Print Current Process Id  \nppid   - Print Parent Process Id\n");
  printf("cd     - Change the Directory      \nquit   - To Exit the Shell\n");
  printf("cat    - For File Handling         \ninfo   - Help About External Commands\n");
  printf("calc   - Scientific Calculator     \ncal    - Very Advanced Calender     \n");
  printf("ntow   - Number to Word Convertor  \nstyle  - Print Name in Style   \n");
  printf("wc     - Advanced Word Count       \nhelp   - Help About Internal Commands\n");
}
