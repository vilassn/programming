...
main()
{
  ...
  chance:
  printf("Previous Month <= p       End       n => Next Month\n");
  printf("Backward Year  <= b        e        f => Forward Year\n\n\t\t\t""Enter:");
  scanf(" %c",&shift);
  system("clear");
  switch(shift)
  {
   ...
   case 'p': mm--;
             if(mm==0)
              ...
              goto prevnext;
   case 'b':yy--;goto prevnext;
   case 'f':yy++;goto prevnext;
   case 'e': return 0;
...
int dayfind(int dd, int mm, int yy)
{
  ...
  y = yy % 400; //9
  cen = y / 100; //0
  if(cen==0) oyear=0; if(cen==1) oyear=5; if(cen==2) oyear=3; if(cen==3) oyear=1;
  y = y % 100; //9
  yr = (y-1)/4; // 8/4 = 2
